...
public class JigoumingchengTag extends TagSupport {
	...
	private void doInit(String schoolCode) throws IOException {
		...
		List<SchoolInfo> schoolInfos = query.list();
		...
		for (SchoolInfo s : schoolInfos) {
			if (value == null) {
				if (s.getCode().equals(schoolCode)) {
					result.add("<option selected=\"selected\" value=\""
							+ s.getCode() + "\">" + s.getName() + "</option>");
				} else {
					result.add("<option value=\"" + s.getCode() + "\">"
							+ s.getName() + "</option>");
				}
			} else {
			...
			code = code + String.valueOf(s.getCode()) + ",";
		}

		if (code.length() > 0) {
			code = code.substring(0, code.length() - 1);
		}
		result.add(0, "<option value=\"" + code + "\">全部机构</option>");
