...
public class TypesInfoTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			Query query = session
					.createQuery("select o from TypesInfo o where o.organization =?");
			query.setParameter(0, schoolCode);
			List<TypesInfo> list = query.list();

			pageContext.getOut().print(
					"<table class=\"showtable\" id=\"typesInfoTable\">");
			for (TypesInfo tInfo : list) {
			...
			pageContext.getOut().print("<tr>");
			pageContext.getOut().print("<td>");
			pageContext.getOut().print("<input type=\"text\"  id=\"types\"/>");
			pageContext
					.getOut()
					.print("<input type=\"button\" id=\"typesButton\" value =\"追加\"/>");
			pageContext.getOut().print("</td>");
			pageContext.getOut().print("</tr>");
